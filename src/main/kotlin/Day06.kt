private const val INPUT = """clvxybjp
kripatlzy
...
object Day06 {
     ...
     * */
    private fun day6section1() = println(
        groups
            .map { it.replace("\\s".toRegex(), "").toSet() }
            .map { it.size }
     ...
     */
    private fun day6section2() {
        val uniqueLetters = INPUT.replace("\\s".toRegex(), "").toSortedSet()
        println(
            groups.map { group ->
                val allAnswersInGroup = mutableListOf<Char>()
                val personalAnswers = group.split("\n").filter { it.isNotBlank() }
                uniqueLetters.forEach { char ->
                    if (personalAnswers.all { it.contains(char) }) {
                        allAnswersInGroup.add(char)
                    }
                }
                return@map allAnswersInGroup.size
            }.fold(0) { sum, element -> sum + element }
        )
    }
}