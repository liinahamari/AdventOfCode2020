private const val INPUT = """byr:2010 pid:#1bb4d8 eyr:2021 hgt:186cm iyr:2020 ecl:grt
...
object Day04 {
     ...
     * */
    private fun day4section1() = println(passports.count {
        it.contains("byr") && it.contains("iyr") && it.contains("eyr") && it.contains("hgt") && it.contains("hcl") && it.contains(
            "ecl"
        ) && it.contains("pid")
    })
     ...
     * */
    private fun day4section2() = println(passports.count {
                it.contains("byr:(([1][9][2][0-9])|([1][9][3-9][0-9])|([2][0][0][0-2]))".toRegex())
                &&
                it.contains("iyr:20(([1][0-9])|([2][0]))".toRegex())
                &&
                it.contains("eyr:20(([2][0-9])|([3][0]))".toRegex())
                &&
                it.contains("hgt:((1(([5-8][0-9])|([9][0-3]))cm)|(((59)|(6[0-9])|(7[0-6]))in))".toRegex())
                &&
                it.contains("hcl:#[0-9a-f]{6}".toRegex())
                &&
                it.contains("ecl:(amb|blu|brn|gry|grn|hzl|oth)".toRegex())
                &&
                it.contains("\\bpid:[0-9]{9}\\b".toRegex())
                &&
                it.contains("pid:000000000").not()
    })
}